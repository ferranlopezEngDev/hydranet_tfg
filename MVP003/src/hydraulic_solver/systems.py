...
class Connection(ABC):
    ...
    def getResultDetails(
        ...
        """Return optional model-specific operating-point details."""
        return {}
...
class HydraulicSystem:
    ...
    __slots__ = ("nodes", "connections", "_nodeConnectionIds")
    ...
    def __init__(
        ...
        self._nodeConnectionIds: dict[str, dict[str, None]] = {}
    ...
    def addNode(self, nodeId: str, node: Node) -> None:
        ...
        self._nodeConnectionIds[nodeId] = {}
    ...
    def removeNode(
        ...
        *,
        ...
        self._nodeConnectionIds.pop(nodeId, None)
    ...
    def addConnectionEntry(
        ...
        self._registerConnectionEndpoints(
            connectionId,
            connectionEntry.node1Id,
            connectionEntry.node2Id,
        )
    ...
    def removeConnection(self, connectionId: str) -> ConnectionEntry:
        ...
        connectionEntry = self.connections.pop(connectionId)
        self._unregisterConnectionEndpoints(
            connectionId,
            connectionEntry.node1Id,
            connectionEntry.node2Id,
        )
    ...
    def replaceConnectionEntry(
        ...
        self._unregisterConnectionEndpoints(
            connectionId,
            previousConnectionEntry.node1Id,
            previousConnectionEntry.node2Id,
        )
        self.connections[connectionId] = connectionEntry
        self._registerConnectionEndpoints(
            connectionId,
    ...
    def getConnectedNodeIds(self) -> tuple[str, ...]:
        ...
        return tuple(
            ...
            if self._nodeConnectionIds.get(nodeId)
        )
    ...
    def getIsolatedNodeIds(self) -> tuple[str, ...]:
        ...
        return tuple(
            nodeId
            for nodeId in self.nodes
            if not self._nodeConnectionIds.get(nodeId)
        )

    def getIncidentConnectionIds(self, nodeId: str) -> tuple[str, ...]:
        """Return the ids of the connections incident to the requested node."""
        self._requireNode(nodeId)
        return tuple(self._nodeConnectionIds[nodeId])
    ...
    def iterConnectionsForNode(
        ...
        for connectionId in self._nodeConnectionIds[nodeId]:
            yield connectionId, self.connections[connectionId]
    ...
    def _validateConnectionReferences(self) -> None:
        ...
        for connectionId, connectionEntry in self.connections.items():
            ...
            if connectionId not in self._nodeConnectionIds[connectionEntry.node1Id]:
                raise ValueError(
                    f"Connection index missing reference from node "
                    f"'{connectionEntry.node1Id}' to connection '{connectionId}'"
                )

            if connectionId not in self._nodeConnectionIds[connectionEntry.node2Id]:
                raise ValueError(
                    f"Connection index missing reference from node "
                    f"'{connectionEntry.node2Id}' to connection '{connectionId}'"
                )
    ...
    def _registerConnectionEndpoints(
        self,
        connectionId: str,
        node1Id: str,
        node2Id: str,
    ) -> None:
        """Add one connection id to the ordered incident index of both nodes."""
        self._nodeConnectionIds[node1Id][connectionId] = None
        self._nodeConnectionIds[node2Id][connectionId] = None

    def _unregisterConnectionEndpoints(
        self,
        connectionId: str,
        node1Id: str,
        node2Id: str,
    ) -> None:
        """Remove one connection id from the ordered incident index."""
        self._nodeConnectionIds[node1Id].pop(connectionId, None)
        self._nodeConnectionIds[node2Id].pop(connectionId, None)
