...
class SampledConnectionTests(unittest.TestCase):
    ...
    def test_factor_polynomial_uses_head_difference(self) -> None:
        connection = FactorPolynomialConnection(
            constantCoefficient=1.0,
            coefficients=(-0.5, 0.25),
            exponents=(1.0, 2.0),
        )

        self.assertAlmostEqual(connection.getOutputValue(2.0), 1.0, places=12)
        self.assertAlmostEqual(connection.getFlowRate(H1=5.0, H2=3.0), 1.0, places=12)

    def test_factor_polynomial_uses_signed_absolute_power_for_negative_inputs(
        self,
    ) -> None:
        connection = FactorPolynomialConnection(
            coefficients=(1.0,),
        ...
        self.assertAlmostEqual(connection.getOutputValue(4.0), 2.0, places=12)
        self.assertAlmostEqual(connection.getFlowRate(H1=4.0, H2=3.0), -1.0, places=12)

    def test_factor_polynomial_rejects_zero_input_for_negative_exponent(self) -> None:
        connection = FactorPolynomialConnection(
            coefficients=(1.0,),
            exponents=(-0.5,),
        )
