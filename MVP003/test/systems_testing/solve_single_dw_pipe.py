"""Solve a one-pipe source-to-demand network with the Darcy-Weisbach model."""
...
EXPECTED_DEMAND_HEAD: float = 98.76469530626343


def build_system() -> HydraulicSystem:
    """Build a minimal valid network with one Darcy-Weisbach pipe."""
...
def solve_and_validate() -> dict[str, object]:
    ...
    if abs(solvedHead - expectedHeadFromLaw) > 1e-10:
        raise AssertionError(
            f"Demand head is inconsistent with the Darcy law: "
