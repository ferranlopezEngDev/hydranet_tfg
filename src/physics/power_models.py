...
def local_power_law_parameters_from_darcy(
    ...
    *,
    ...
    `relativeBand` controls the width of the local fitting interval:
    `Q1 = |Q| * (1 - relativeBand)` and `Q2 = |Q| * (1 + relativeBand)`,
    with the lower bound clipped by `minimumFlowRate`. Smaller values
    make the fit more local; larger values smooth it over a wider range.
    """
    length, diameter, kinematicViscosity = validate_pipe_geometry(
        length=length,
        diameter=diameter,
